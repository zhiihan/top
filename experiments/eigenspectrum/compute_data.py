...
def make_Hamiltonian(N,J,h,t,mu,Delta,k):
    ...
    spin_basis = quspin.basis.spin_basis_1d(N,kblock=k)    
    fermion_basis = quspin.basis.spinless_fermion_basis_1d(N,kblock=k)
...
def make_Magnetisation_staggered(N,k):
    """Returns a Quspin Quantum Operator. (Hamiltonian)"""
...
def compute_data(N, Delta = 1, t = 1, mu = 0, J = 1, h = 0,k=None):
    """Returns a Python dictionary."""
    
    #print('Beginning trial N = ',N,'h = ',h,'J = ',J, 'Delta =', Delta)

    H = make_Hamiltonian(N,J,h,t,mu,Delta,k)
    ...
    M = make_Magnetisation(N,k)
    O = make_Fermion_pair(N,k)
    Ms = make_Magnetisation_staggered(N,k)
    V = V[:,0]
    V = V[:, np.newaxis]
...
for N in range(7, 13):
    for k in range(N):
        for Delta in np.linspace(0,2,num=20):
            for J in np.linspace(0,1,num=20):
                try:
                    list_dicts.append(compute_data(N, Delta=Delta, t=1, mu=0, J=J, h=h, k=k))
                    print('Successful', {'N':N, 'h':h,  'Delta':Delta, 'J':J, 'k':k})
                    ...
                    print('Convergence failed', {'N':N, 'h':h,  'Delta':Delta, 'J':J, 'k':k})
                    ...
                    print('ARPACK Error', {'N':N, 'h':h,  'Delta':Delta, 'J':J, 'k':k})

    df = pd.DataFrame(list_dicts)
    pd.DataFrame.to_csv(df,'quspin_eigenk'+str(N)+'.csv',index=False)